...
class AttentionSuper(nn.Module):
    ...
    def set_normalized_arch_weights(self, embed_weights, n_heads_weights):
        self.embed_alphas = embed_weights
        self.n_heads_alphas = n_heads_weights
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x, self.embed_alphas, self.n_heads_alphas).reshape(B, N, 3, self.sample_num_heads, -1).permute(2, 0, 3, 1, 4) # (3, B, num_heads, N, 64)
        ...
        x = self.proj(x, self.embed_alphas, self.embed_alphas)
        x = self.proj_drop(x)
        return x
